                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: TextField(
                            onChanged: (value) {
                              setState(() {
                                filteredPlayers =
                                    _filterPlayers(value, searchCriteria, listOfPlayersFromApi, teams);
                              });
                            },
                        ...
                        IconButton(
                          ...
                          onPressed: () async {
                            String? selectedCriteria = await _showSearchCriteriaDialog(context);
...
Future<String?> _showSearchCriteriaDialog(BuildContext context) {
  return showDialog<String>(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text('Select Search Criteria'),
        content: Column(
...
List<Player> _filterPlayers(String value, String searchCriteria, List<Player> listOfPlayers, List<Team> teams) {
  switch (searchCriteria) {
    case 'Search by Player Name':
      return listOfPlayers.where((player) => player.name.toLowerCase().contains(value.toLowerCase())).toList();
    case 'Search by Player Position':
      return listOfPlayers.where((player) => player.position.toLowerCase().contains(value.toLowerCase())).toList();
    case 'Search by Player Price':
    // You can implement price filtering logic here
      return listOfPlayers;
    case 'Search by Team Name':
      return listOfPlayers.where((player) => teams.any((team) => team.id == player.teamId && team.name!.toLowerCase().contains(value.toLowerCase()))).toList();
    default:
      return listOfPlayers;
  }
}
